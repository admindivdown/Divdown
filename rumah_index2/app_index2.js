...
/* === AMBIL DARI SESSION / SERVER === */ try{let data=JSON.parse(sessionStorage.getItem('fbData')||'null');
                                      if(!data){const res=await fetch('https://divdown-production.up.railway.app/api/facebook?url='+encodeURIComponent(fbUrl));data=await res.json();}
                                              if(data.success){
...
window.open('https://divdown-production.up.railway.app/api/download?url='+encodeURIComponent(data.standard),'_blank')
