...
/* === AMBIL DARI SESSION / SERVER === */ try{let data=JSON.parse(sessionStorage.getItem('fbData')||'null');if(!data){const res=await fetch('/api/facebook?url='+encodeURIComponent(fbUrl));data=await res.json();}
if(data.success){
...
/* === DOWNLOAD VIDEO === */
if(btnStandard){if(data.standard&&data.standard.trim()){btnStandard.onclick=()=>{window.open('/api/download?url='+encodeURIComponent(data.standard),'_blank')
...
window.open('/api/download?url='+encodeURIComponent(data.hd720),'_blank')
