window.brazil = {

  /* ===== DATA UTAMA ===== */
  title: "Divdown.net - Baixador de Vídeos Rápido | Grátis e Sem Login",
  placeholder: "Cole o link do vídeo aqui...",
  bendera: "🇧🇷",
  nama: "Português",

  /* ===== PERGUNTAS FREQUENTES ===== */
  faqTitle: "Perguntas Frequentes",
  faqQ1: "O Divdown é seguro e privado?",
  faqA1: "Sim. O Divdown é seguro e privado. Usamos criptografia SSL, não é necessário fazer login e nunca armazenamos seus dados ou arquivos em nossos servidores. Todo o processamento é feito diretamente no seu navegador.",
  faqQ2: "O Divdown é gratuito para usar?",
  faqA2: "Sim. O Divdown é 100% gratuito, sem custos ocultos, sem assinaturas e com downloads ilimitados.",
  faqQ3: "Posso baixar em qualidade HD?",
  faqA3: "Sim. O Divdown suporta qualidade HD de até 1080p, 2K e 4K quando disponível no arquivo original. Funciona em celular, tablet e computador.",
  faqQ4: "Preciso fazer login para usar?",
  faqA4: "Não é necessário login. Basta colar o link de um vídeo ou Reels público e salvá-lo diretamente no seu dispositivo. Nunca pedimos senhas ou dados da conta.",
  faqQ5: "Por que meu vídeo não foi processado?",
  faqA5: "Verifique se o link está correto, se o vídeo é público e se sua conexão com a internet está estável. O Divdown só pode processar vídeos que estejam acessíveis publicamente.",

  /* ===== SOBRE O DIVDOWN ===== */
  aboutTitle: "Sobre o Divdown",
  aboutText1: "O Divdown é uma plataforma online segura que ajuda você a salvar vídeos em qualidade HD para assistir offline. Nosso serviço é 100% gratuito, rápido e não requer cadastro, login ou instalação de programas.",
  aboutText2: "Desenvolvido com uma interface leve, limpa e adaptada para celulares, o Divdown garante uma experiência de uso fluida e privada. Respeitamos sua privacidade: não coletamos dados pessoais e todos os downloads são processados de forma segura no seu navegador.",

  /* ===== FALE CONOSCO ===== */
  contactTitle: "Fale Conosco",
  contactText1: "Se tiver dúvidas, relatar erros ou enviar propostas comerciais, entre em contato conosco por:",

  /* ===== POLÍTICA DE PRIVACIDADE ===== */
  privacyTitle: "Política de Privacidade",
  privacyText1: "No Divdown, temos o compromisso de proteger sua privacidade. Esta política explica como tratamos as informações quando você usa nosso serviço para salvar vídeos públicos para uso pessoal e visualização offline.",
  privacySub1: "Informações que Não Coletamos",
  privacyText2: "Não é necessário criar conta ou fazer login para usar o Divdown. Não coletamos, armazenamos ou compartilhamos dados pessoais, senhas ou endereços de e-mail de nenhum usuário.",
  privacySub2: "Links de Vídeo e Processamento",
  privacyText3: "Os links enviados por você são usados apenas para realizar o download solicitado. Todo o processamento ocorre de forma segura no seu navegador. Não guardamos, armazenamos ou mantemos cópias de nenhum vídeo ou endereço em nossos servidores.",
  privacySub3: "Cookies",
  privacyText4: "O Divdown pode usar cookies essenciais para garantir o funcionamento correto do site e analisar o tráfego de forma anônima para melhorar o desempenho. Esses cookies não contêm dados pessoais e podem ser desativados nas configurações do seu navegador.",
  privacySub4: "Anúncios que Ajudam a Manter o Serviço",
  privacyText5: "Para manter o Divdown gratuito e em constante melhoria, exibimos anúncios de parceiros confiáveis. Esses anúncios ajudam a cobrir os custos dos servidores, permitindo que você baixe sem limites. Não mostramos anúncios invasivos ou que atrapalhem o uso.",
  privacySub5: "Atualizações da Política",
  privacyText6: "Podemos atualizar esta política de tempos em tempos, conforme mudanças em nossos serviços ou por exigências legais. Qualquer alteração será publicada nesta página com a data de revisão. Recomendamos que você consulte esta página periodicamente.",

  /* ===== TERMOS DE SERVIÇO ===== */
  termsTitle: "Termos de Serviço",
  termsText1: "Seja bem-vindo ao Divdown. Ao acessar ou usar este site, você concorda em utilizar nosso serviço de forma responsável e apenas para fins pessoais e legais.",
  termsSub1: "Sobre o Serviço",
  termsText2: "O Divdown é uma ferramenta online que ajuda você a salvar vídeos disponíveis publicamente para assistir offline. O serviço é gratuito, rápido, leve e funciona em todos os tipos de dispositivos.",
  termsSub2: "Uso Permitido",
  termsText3: "Você concorda em usar o Divdown apenas para fins pessoais e não comerciais. É sua responsabilidade garantir que tenha permissão para salvar e assistir ao conteúdo que processar. O Divdown não hospeda nenhum vídeo e trabalha apenas com os links públicos que você fornece.",
  termsSub3: "Alterações no Serviço",
  termsText4: "Reservamos o direito de alterar, interromper ou encerrar qualquer parte do serviço a qualquer momento, para manter a qualidade, segurança e boa experiência de uso. As funcionalidades podem ser atualizadas sem aviso prévio.",
  termsSub4: "Isenção de Responsabilidade",
  termsSub5: "Isenção de Responsabilidade",
  termsText5: "Estamos sempre aprimorando o Divdown para mantê-lo rápido, seguro e fácil de usar. Ao usar este serviço, você concorda em utilizá-lo de forma responsável e apenas para fins pessoais. Não permitimos o uso do Divdown para atividades que violem direitos autorais ou leis vigentes."

};
